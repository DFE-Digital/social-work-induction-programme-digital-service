...
function setup_DB() {
    ...
    try {
        ...
        print_r("Failed to connect to DB server... " . $CFG->dbhost);
        print_r("DB library: " . $CFG->dblibrary);
        print_r("DB type: " . $CFG->dbtype);
        print_r("Database: " . $CFG->dbname);
        print_r("User: " . $CFG->dbuser);
        print_r("Password: " . (!empty($CFG->dbpass) ? "Is specified..." : "Not specified..."));
        print_r("DB prefix: " . $CFG->prefix);
        $result = dns_get_record($CFG->dbhost);
        print_r("DNS results: " . json_encode($result));
        $connection = @fsockopen($CFG->dbhost, 5432);
        print_r("Socket open results: " . json_encode($connection));
      
        if (is_resource($connection)) {
          print_r("DNS results: " . json_encode($result));
          fclose($connection);
        } else {
          print_r("Host is not responding");
        }
