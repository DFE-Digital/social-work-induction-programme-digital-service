...
public class CreateUserAsyncShould : MoodleServiceTestBase
{
    [Fact]
    public async Task WhenCalled_SuccessfulRequest_ReturnsCreatedExternalUserId()
    {
        ...
        MockClient.Setup(x => x.User.CreateAsync(MoqHelpers.ShouldBeEquivalentTo(moodleRequest))).ReturnsAsync(moodleResponse);

        // Act
        var response = await Sut.CreateUserAsync(accountDetails);

        // Assert
        response.Should().Be(moodleResponse.Id);
        MockClient.Verify(x => x.User.CreateAsync(MoqHelpers.ShouldBeEquivalentTo(moodleRequest)), Times.Once);
    ...
    public async Task WhenCalled_UnsuccessfulRequest_ReturnsNull()
    {
        ...
        var actualException = await Assert.ThrowsAsync<Exception>(
            async () => await Sut.CreateUserAsync(accountDetails)
        );

        // Assert
        actualException.Message.Should().Be($"Failed to create Moodle user with email {moodleRequest.Email}");
