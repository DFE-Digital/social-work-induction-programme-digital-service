...
public class GetRouteIntoSocialWorkShould : RegisterSocialWorkerJourneyServiceTestBase
{
    [Fact]
    public async Task WhenCalled_ReturnRouteIntoSocialWork()
    {
        // Arrange
        var id = Guid.NewGuid();
        ...
        // Act
        var response = await Sut.GetRouteIntoSocialWorkAsync(id);

        // Assert
        response.Should().NotBeNull();
        response.Should().Be(expected.RouteIntoSocialWork);
