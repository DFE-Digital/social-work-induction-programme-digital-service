...
public class GetHighestQualificationShould : RegisterSocialWorkerJourneyServiceTestBase
{
    [Fact]
    public async Task WhenCalled_ReturnHighestQualification()
    {
        // Arrange
        var id = Guid.NewGuid();
        ...
        // Act
        var response = await Sut.GetHighestQualificationAsync(id);

        // Assert
        response.Should().NotBeNull();
        response.Should().Be(expected.HighestQualification);
