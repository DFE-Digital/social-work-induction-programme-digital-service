...
public class UnpauseAccountPageTests : ManageAccountsPageTestBase<UnpauseAccount>
{
    private UnpauseAccount Sut { get; }

    public UnpauseAccountPageTests()
    {
        Sut = new UnpauseAccount(
    ...
    public async Task Post_WhenCalled_UpdatesAccountStatusAndRedirectsToAccountDetails()
    {
        ...
        MockEditAccountJourneyService
            .Setup(x => x.GetIsStaffAsync(account.Id))
            .ReturnsAsync(account.IsStaff);
        ...
        MockEditAccountJourneyService.Setup(x => x.SetAccountStatusAsync(account.Id, statusValue));
        ...
        MockEditAccountJourneyService.Verify(
            x => x.SetAccountStatusAsync(account.Id, statusValue),
            Times.Once
        );
