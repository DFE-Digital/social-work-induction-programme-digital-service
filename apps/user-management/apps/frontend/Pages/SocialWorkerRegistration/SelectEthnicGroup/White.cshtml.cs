...
public class SelectWhiteEthnicGroup(
    EcfLinkGenerator linkGenerator,
    IRegisterSocialWorkerJourneyService socialWorkerJourneyService,
    IAuthServiceClient authServiceClient,
    IValidator<SelectWhiteEthnicGroup> validator)
    : BasePageModel
{
    [BindProperty] public EthnicGroupWhite? SelectedEthnicGroupWhite { get; set; }
    [BindProperty] public string? OtherWhiteEthnicGroup { get; set; }

    public async Task<PageResult> OnGetAsync()
    {
        var personId = authServiceClient.HttpContextService.GetPersonId();
        SelectedEthnicGroupWhite = await socialWorkerJourneyService.GetWhiteEthnicGroupAsync(personId);
        OtherWhiteEthnicGroup = await socialWorkerJourneyService.GetOtherWhiteEthnicGroupAsync(personId);
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        await socialWorkerJourneyService.SetOtherWhiteEthnicGroupAsync(personId, OtherWhiteEthnicGroup);
