    ...
    EcfLinkGenerator linkGenerator) : ManageAccountsBasePageModel
{
    ...
    private bool HasAccountDetails()
    {
        ...
        if (accountDetails is null) return false;

        // ECSWs have a SWE ID but no account details at this stage
        if (isEcsw && string.IsNullOrWhiteSpace(accountDetails.Email)) return false;

        // Other types don't have SWE ID or account details
        if (accountDetails.SocialWorkEnglandNumber is null) return false;
