...
internal static class EmbeddedSqlHelper
{
    ...
    {
        var resourceName = $"{typeof(EcfDbContext).Namespace}.{relativePath}";
        using var resourceStream = typeof(EcfDbContext).Assembly.GetManifestResourceStream(resourceName) ??
