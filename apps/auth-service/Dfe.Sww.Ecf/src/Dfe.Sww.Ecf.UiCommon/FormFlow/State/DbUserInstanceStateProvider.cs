    ...
    IOptions<JsonOptions> jsonOptionsAccessor) : DbUserInstanceStateProviderBase(clock, jsonOptionsAccessor)
{
    ...
    private async Task<EcfDbContext> EnsureDbContext()
    {
        var httpContext = httpContextAccessor.HttpContext ?? throw new InvalidOperationException("No HttpContext.");
    ...
    private bool TryGetDbContext(HttpContext httpContext, [NotNullWhen(true)] out EcfDbContext? dbContext)
    {
        if (httpContext.Items.TryGetValue(HttpContextItemsDbContextKey, out var dbContextObj) && dbContextObj is EcfDbContext dbc)
        {
            dbContext = dbc;
            return true;
        }
