...
public abstract class DbUserInstanceStateProviderBase(IClock clock, IOptions<JsonOptions> jsonOptionsAccessor) : IUserInstanceStateProvider
{
    ...
    protected async Task CompleteInstanceAsync(JourneyInstanceId instanceId, Type stateType, string userId, EcfDbContext dbContext)
    {
    ...
    protected async Task DeleteInstanceAsync(JourneyInstanceId instanceId, Type stateType, string userId, EcfDbContext dbContext)
    {
        var instance = await dbContext.JourneyStates.SingleOrDefaultAsync(j => j.InstanceId == instanceId && j.UserId == userId) ??
            throw new ArgumentException("Instance does not exist.");
    ...
    protected async Task<JourneyInstance?> GetInstanceAsync(JourneyInstanceId instanceId, Type stateType, string userId, EcfDbContext dbContext)
    {
        var instance = await dbContext.JourneyStates.SingleOrDefaultAsync(j => j.InstanceId == instanceId && j.UserId == userId);
    ...
    protected async Task UpdateInstanceStateAsync(JourneyInstanceId instanceId, Type stateType, object state, string userId, EcfDbContext dbContext)
    {
        var instance = await dbContext.JourneyStates.SingleOrDefaultAsync(j => j.InstanceId == instanceId && j.UserId == userId) ??
            throw new ArgumentException("Instance does not exist.");
