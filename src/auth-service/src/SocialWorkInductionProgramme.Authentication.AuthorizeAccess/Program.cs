...
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.Infrastructure.Filters;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.Infrastructure.FormFlow;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.Infrastructure.Logging;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.Infrastructure.Security;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.Infrastructure.Security.Configuration;
using SocialWorkInductionProgramme.Authentication.AuthorizeAccess.TagHelpers;
...
using SocialWorkInductionProgramme.Authentication.Core.Services.Accounts;
using SocialWorkInductionProgramme.Authentication.ServiceDefaults;
using SocialWorkInductionProgramme.Authentication.SupportUi.Infrastructure.FormFlow;
using SocialWorkInductionProgramme.Authentication.UiCommon.Filters;
using SocialWorkInductionProgramme.Authentication.UiCommon.FormFlow;
using SocialWorkInductionProgramme.Authentication.UiCommon.Middleware;
...
var oneLoginConfigurationSection = builder.Configuration.GetRequiredSection(
    OneLoginConfiguration.ConfigurationKey
);
var oneLoginConfiguration = oneLoginConfigurationSection.Get<OneLoginConfiguration>();
if (oneLoginConfiguration is null)
{
    throw new InvalidConfigurationException("Missing OneLogin configuration.");
}
if (oneLoginConfiguration.Url is null)
{
    throw new InvalidConfigurationException("OneLogin Url not configured.");
}
if (oneLoginConfiguration.ClientId is null)
{
    throw new InvalidConfigurationException("OneLogin ClientId not configured.");
}
builder.Services.Configure<OneLoginConfiguration>(oneLoginConfigurationSection);
...
builder
    ...
    .AddOneLogin(options =>
    {
        ...
        options.MetadataAddress = oneLoginConfiguration.Url + "/.well-known/openid-configuration";
        options.ClientAssertionJwtAudience = oneLoginConfiguration.Url + "/token";

        using (var rsa = RSA.Create())
        {
            var privateKeyPem = oneLoginConfiguration.PrivateKeyPem;
